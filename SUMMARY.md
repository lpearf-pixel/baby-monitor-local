...
- Visual regression corpus work is published on `codex/visual-regression-corpus`; the
  ...
  with implementation plan in the adjacent plans directory. Task 1 commit
  ...
  household media, real descriptor, replay or baseline action occurred. Software Tasks
  1–7 and the independent review fixes are published on the current branch. The
...
- The approved offline Guardian scenario flow is implemented and published through
  ...
  Its reviewed form adds independent `prone_candidate` and `outside_candidate`
  Guardian oracles plus generated diaper and burping Voice scenarios, corrects
  synthetic visual provenance and keeps actual visual output non-ground-truth. The
  owner paused before plan or implementation; no expansion code, fixture, report or
...
combined single-sentence form. The separately approved ContextualParaformer/hotword A/B
has now completed and was rejected: baseline and candidate each reached 21/24 positives,
48/48 negative rejects and zero false accepts; both missed the three
medication-complete positives. Candidate p95 was 294 ms and RSS was 1,666,121,728 bytes,
but the mandatory public gate still failed. The private-local gate was therefore not
opened and production remained on the current Paraformer. Do not widen ASR correction
from household evidence, enable Camera Reply, delete retained private sessions or
