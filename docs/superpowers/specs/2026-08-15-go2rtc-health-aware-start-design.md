...
1. If the API is healthy, keep the existing process and continue startup.
...
4. If the API is unhealthy and the PID is alive, verify through `ps` that the
...
- a healthy API causes no restart;
