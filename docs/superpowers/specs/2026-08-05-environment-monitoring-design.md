...
透视校正画布必须保留已标定仪表四边形的实际纵横比：输出宽度取上下边长度的
较大值，输出高度取左右边长度的较大值，并受原始帧尺寸上限约束。归一化点先按
原始 source 宽高换算，再进入透视矩阵；表盘边界、搜索窗口和圆检测则使用校正后
画布宽高。禁止把仪表四边形强制拉伸到摄像头 16:9 全帧尺寸，因为这会把圆形
表盘变形并错误触发 `roi_out_of_bounds`。退化边长或越界输出继续返回
