...
3. 完全黑暗/红外、反光、短时遮挡和移动表盘分别验证；夜间达不到误差目标时
   必须稳定输出不可用，不虚报数值；
4. 连续运行 12 小时，60 秒调度无队列增长，趋势和空档正确；
...
该固定提交的官方 trainer 将设备硬编码为 CUDA，不能在 Intel i9 macOS 上运行。
本项目不修改上游源码；使用同一提交的 YOLOX-Tiny 模型、损失和优化器，通过独立、
固定依赖的 CPU 训练循环完成本地训练。训练环境位于忽略的 runtime 目录，不进入
常驻 Guardian 依赖；W&B、远程日志和训练期网络访问固定关闭。
