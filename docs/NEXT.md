...
It uses `DAY-03` and `OCC-03` only as independent pipeline observations, separate from
synthetic `prone_candidate` and `outside_candidate` Guardian oracles, and uses generated
inputs for the approved low-risk diaper/burping Voice actions. The owner paused before
plan and implementation. When explicitly resumed, write the implementation plan first;
do not change Guardian rules, enable Camera Reply or write Baby Care. Real-baby accuracy
and public baseline promotion remain separate later gates.
