...
help:
	...
	@echo "  make alpha-voice-contextual-ab Run generated aggregate-only contextual ASR A/B"
	@echo "  make alpha-voice-contextual-ab-private Run public gate then retained private aggregate"
	...
	@echo "  make alpha-visual-regression-long Run the deferred 30-minute replay entry"
	@echo "  make alpha-offline-scenario-validate Validate fixed offline Guardian scenarios"
	@echo "  make alpha-offline-scenario-run Run the isolated offline Guardian scenario flow"
	@echo "  make alpha-offline-application-validate Validate the fixed offline application rehearsal"
	@echo "  make alpha-offline-application-run Run the fixed offline application rehearsal"
...
alpha-xiaomi-media-diagnostic:
	@$(PYTHON) tools/xiaomi_media_diagnostic.py
...
alpha-voice-contextual-ab:
	@$(PYTHON) -m tools.voice_contextual_ab

alpha-voice-contextual-ab-private:
	@$(PYTHON) -m tools.voice_contextual_ab --private
...
alpha-visual-corpus-validate:
	@$(PYTHON) tools/visual_corpus.py validate

alpha-visual-corpus-prepare:
	@$(PYTHON) tools/visual_corpus.py prepare --first-stage
...
alpha-visual-regression-compare:
	@$(PYTHON) tools/visual_corpus.py compare

alpha-visual-regression-promote:
	@$(PYTHON) tools/visual_corpus.py promote --expected-digest "$(BASELINE_SHA256)"

alpha-visual-regression-long:
	@$(PYTHON) tools/visual_corpus.py long --minutes 30

alpha-offline-scenario-validate:
	@$(PYTHON) tools/offline_guardian_scenario.py validate

alpha-offline-scenario-run:
	@$(PYTHON) tools/offline_guardian_scenario.py run

alpha-offline-application-validate:
	@$(PYTHON) tools/offline_application_rehearsal.py validate

alpha-offline-application-run:
